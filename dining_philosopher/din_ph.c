...
//create # philosopher & # spoons
//philosophers will be thread & spoons will be shared resources

static phil_t phil[N_PHILOSOPHER];
static spoon_t spoon[N_PHILOSOPHER];

/*define 3 func for 3 actions for philosopher which doesn't deal with any CS*/

//#1 -> get the right spoon
//returns spoon pointer
...
//#2 ->get the left spoon
//return spoon pointer
...
void phil_eat(phil_t *phil){
    ...
    /*checking if philospher has got the right spoons*/
    /*program will crash if any of these asserts to false*/
    ...
    printf("Phil %d eats with spoon [%d, %d] for %d times\n",
		...
		phil->eat_count);
    sleep(1); /* let the philosopher eat for 1 sec -> constraint */
}
